...
function updateAdminStats() {
    ...
    let totalRevenue = 0;
    orders.forEach(ord => {
        ord.items.forEach(item => { totalRevenue += item.price; });
    });
    let totalProducts = products.length;

    let ordElem = document.getElementById('adminTotalOrders');
    let revElem = document.getElementById('adminTotalRevenue');
    let prodElem = document.getElementById('adminTotalProds');

    if(ordElem) ordElem.innerText = totalOrders;
    if(revElem) revElem.innerText = '₹' + totalRevenue;
